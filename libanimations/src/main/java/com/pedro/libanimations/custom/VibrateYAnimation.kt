...
class VibrateYAnimation(val delay: Int = 0) : BaseAnimation() {
    ...
    private fun moveMore(v: View?): ObjectAnimator {
        val animatorY = ObjectAnimator.ofFloat(v, "translationY", 3f)
        animatorY.duration = 20
        state = true
        return animatorY
    }

    private fun moveLess(v: View?): ObjectAnimator {
        val animatorX = ObjectAnimator.ofFloat(v, "translationY", -3f)
